...
class NEWDupeFilter(RFPDupeFilter):
    ...
    def request_seen(self, request):
        ...
        try:
            ...
            mark = self.server.hget('del_dup', fp)
            request_time = datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')
            if mark == None:
                self.server.hset('del_dup', fp,\
                            json.dumps({'request_time': request_time,
                                        'hash': 'null'}))
                return False
            ...
            print("the NEWDupeFilter is wrong :")
            print(e)
            ti.sleep(5)
